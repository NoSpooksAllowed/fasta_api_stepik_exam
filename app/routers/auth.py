...
async def authenticate_user(db: Annotated[AsyncSession, Depends(get_db)], username: str, password: str):
    ...
    if not user or not bcrypt_context.verify(password, str(user.hashed_password)) or bool(user.is_active) == False:
        raise HTTPException(
...
async def get_current_user(token: Annotated[str, Depends(oauth2_scheme)]):
    try:
        ...
        is_admin: str | None = payload.get('is_admin')
        is_supplier: str | None = payload.get('is_supplier')
        is_customer: str | None = payload.get('is_customer')
